...
export const updatePost = async (req, res) => {
    const { id } = req.params;
    const { title, description, resume, category, autor, selectedFile, tags } = req.body;
...
export const deletePost = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`Ninguna noticia con el id: ${id}`);
